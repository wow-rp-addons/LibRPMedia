...
function LRPM12:RequestLoadMusicData()
    for _, subtable in pairs(self.db.music) do
...
function LRPM12:RequestLoadIconData()
    for _, subtable in pairs(self.db.icons) do
        if type(subtable) == "table" then
            nop(subtable[nil]);
        end
    end
...
function LRPM12:PlayMusic(musicID, ...)
    ...
    if musicInfo then
        return true, PlayMusic(musicInfo.filePath or musicInfo.fileID, ...);
    else
        return false;
    end
end
...
function LRPM12:PlayMusicSoundFile(musicID, ...)
    ...
    if musicInfo then
        return true, PlaySoundFile(musicInfo.filePath or musicInfo.fileID, ...);
    else
        return false;
    end
end
...
function LRPM12:ResolveIcon(icon)
    ...
    if not fileID then
        fileID = GetFileIDFromPath([[Interface\ICONS\]] .. tostring(icon));
    end
    ...
    if not fileID or not GetIconIndexByID(self.db.icons, bit.band(fileID, ICON_ID_DATA_MASK)) then
        fileID = INV_MISC_QUESTIONMARK;
    end

    return fileID, LRPM12.IconType.File;
end
...
function GetIconInfoByIndex(iconsDB, iconIndex, infoTable)
    ...
    local fileID = (type == LRPM12.IconType.File) and bit.band(id, ICON_ID_DATA_MASK) or nil;
    local atlasID = (type == LRPM12.IconType.Atlas) and bit.band(id, ICON_ID_DATA_MASK) or nil;
    local key = (type == LRPM12.IconType.Atlas or IsIconFileName(name)) and name or fileID;
...
function GetMusicIndexByName(musicDB, musicName)
    ...
    return musicIndexByName[string.lower(musicName)];
end
...
function GetMusicInfoByIndex(musicDB, musicIndex, infoTable)
    ...
    local fileID = musicDB.file[musicIndex];
    local duration = musicDB.time[musicIndex];
    local filePath;

    if WOW_PROJECT_ID == WOW_PROJECT_CLASSIC then
        filePath = musicDB.name[GetMusicNameIndexRange(musicDB, musicIndex)];
    end
