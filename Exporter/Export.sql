...
(
    Id INTEGER PRIMARY KEY,
    Name TEXT NOT NULL, -- Would ideally be UNIQUE but Blizzard ¯\_(ツ)_/¯
...
CREATE INDEX UiTextureAtlasFile ON UiTextureAtlas (FileId);
CREATE INDEX UiTextureAtlasMemberParent ON UiTextureAtlasMember (AtlasId);
...
SELECT
    ...
    NULLIF(CAST("Sounds[0]" AS INTEGER), 0) AS SoundKitIdDay,
    NULLIF(CAST("Sounds[1]" AS INTEGER), 0) AS SoundKitIdNight
FROM
    CsvZoneMusic
WHERE
...
INSERT INTO UiTextureAtlasMember (Id, Name, AtlasId, Left, Right, Top, Bottom)
SELECT
    CAST(ID AS INTEGER) AS Id,
    NULLIF(GetNormalizedAtlasName(CommittedName), '') AS Name,
...
WHERE
    Name IS NOT NULL AND AtlasId IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS MusicAttribute
(
    ContentHash TEXT PRIMARY KEY,
    Duration REAL NOT NULL
);

CREATE TABLE IF NOT EXISTS IconAttribute
(
    ContentHash TEXT PRIMARY KEY,
...
FROM
    temp.File
WHERE
    Path LIKE 'interface/icons/%.blp' AND NOT IsIconFileExcluded(Id, Path, ContentHash)
ORDER BY
    Id ASC;

CREATE TEMPORARY VIEW IconAtlas (Id) AS
SELECT
    UiTextureAtlasMember.Id
FROM
...
SELECT
    ...
    File.Id,
    File.ContentHash,
    GetNameForIconAtlas(UiTextureAtlasMember.Id, UiTextureAtlasMember.Name),
...
FROM
    IconAtlas
INNER JOIN
    UiTextureAtlasMember ON UiTextureAtlasMember.Id = IconAtlas.Id
INNER JOIN
...
-- Attribute Caching
--
-- These queries are eagerly evaluated and are responsible for updating our
